<h1 align="center">веб-сервис</h1>
    ...
    Также приложение умеет работать с сессиями разных пользователей, а также сохранять их действия при следующем запуске.
    </h3>
...
<h2>шаг 1 </h2>
<h4>скачать веб сервис через git clone
<div class="highlight highlight-source-shell notranslate position-relative overflow-auto" dir="auto"><pre>git clone https://github.com/konodop/GO_Calc</pre></div>
или просто скачать и распаковать .zip файл из гитхаба если не установлен git
</h4>
<h2>шаг 2</h2>
<h4> Запуск сервера. В основной папке с помщью терминала либо git bash ввести команду:</h4>
...
<h2>шаг 3</h2>
<h4> Запуск агента. В той же папке с помщью терминала либо git bash ввести команду (не закрывать прошлый шаг, а в новом терминале запускать):</h4>
...
<h2>шаг 4</h2>
<h4>Снова открываем командную строку с помощью которой можно будет отправлять запросы. Регистрация:</h4>
...
<h2>шаг 5</h2>
<h4> Отправка POST-запроса через curl.</h4>
...
<h2>шаг 6</h2>
    <h4>Смотрим ответ этой задачи</h4>
...
<h2>1</h2>
<h4>
<div class="highlight highlight-source-shell notranslate position-relative overflow-auto" dir="auto"><pre>curl -X POST http://localhost:8080/api/v1/calculate -H "Content-Type: application/json" -H "Authorization: <токен>" -d "{\"expression\": \"1-(1000*10)\"}"</pre></div>
Ответ:
...
<h2>2</h2>
<h4>
<div class="highlight highlight-source-shell notranslate position-relative overflow-auto" dir="auto"><pre>curl -X POST http://localhost:8080/api/v1/calculate -H "Content-Type: application/json" -H "Authorization: <токен>" -d "{\"expression\": \"1/0\"}"</pre></div>
Ответ:
...
<h4>
<div class="highlight highlight-source-shell notranslate position-relative overflow-auto" dir="auto"><pre>curl -X GET http://localhost:8080/api/v1/expressions -H "Authorization: <токен>"</pre></div>
Ответ:
...
<hr></hr>
<h1>Фронтенд(WIP)</h1>
<h4>позволяет по url (http://localhost:8081/) видеть отправленные задачи. Чтобы его увидеть запустите front.exe или напишите эту команду в папке cmd/front:</h4>
...
<hr>
<h1>Тесты</h1>
<h3>Нужно зайти в папку с кодом который нужно проверить и написать в терминал go test -v</h3>
<hr></hr>
